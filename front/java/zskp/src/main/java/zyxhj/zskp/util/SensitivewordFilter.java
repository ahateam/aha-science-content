...
//                "的可装配性，确保装配工序简单猎枪弹夹紧双腿联系方式VX、装配效率高、装配qq质量高、装配不良率低和装配成本低。面向装配的设计通过一系" +
...
//    }
//    
    public String StringFilterMain(String filterText) {

        SensitivewordFilter filter = new SensitivewordFilter();
        Set<String> set = filter.getSensitiveWord(filterText, 1);
        // ------------------------替换敏感字begin----------------------
        Iterator<String> iterator = set.iterator();
        String word = null;
        while (iterator.hasNext()) {
             ...
             * 或全部为replaceChar的关键词
             * */
            filterText = filterText.replaceAll(word, replaceString);
        }
        // ------------------------替换敏感字end----------------------
        return filterText;
    }
